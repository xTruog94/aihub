import transformers
import pandas as pd
from transformers import AutoTokenizer, AutoModelForTokenClassification, ElectraTokenizer,ElectraForTokenClassification,AdamW
...
def load_part_data(input_path,lb_path):
    ...
    with open(lb_path, 'r') as lines:
        for line in lines:
            list_lb = line.replace('\n','').split()
            list_lb = [int(i) for i in list_lb if i != '']
            root_labels.append(list_lb)

    assert len(texts) == len(root_labels)
    text_ids = []
    att_masks = []
    labels = []
    for i,text in enumerate(texts):
        ...
        tmp_lbs = []
        tmp_input_id = []
        ### xử lý trường hợp tokenize 1 từ thành 2 từ
        for idx,word in enumerate(text_split):
            encode_id = tokenizer.encode(word)[1:-1]
            lb_w = [lbs[idx]]*len(encode_id)
            tmp_input_id = tmp_input_id + encode_id
            tmp_lbs = tmp_lbs + lb_w
        #cut to batch_size
        text_id = tmp_input_id[0:MAX_LEN-2]
        tmp_lbs = tmp_lbs[0:MAX_LEN-2]
        # add ids 2 and 3
        text_id = [2] + text_id + [3]
        tmp_lbs = [0] + tmp_lbs + [0]
        current_len = len(text_id)
        # padding
        if current_len < MAX_LEN:
            text_id += [0]*(MAX_LEN - current_len)
            tmp_lbs += [0]*(MAX_LEN - current_len)
        att_mask = [int(token_id>0) for token_id in text_id]

        assert len(text_id) == len(att_mask) == len(tmp_lbs)
...
if __name__ == "__main__":
    ...
    model = ElectraForTokenClassification.from_pretrained(pre_trained, num_labels=num_labels,
    ...
    if os.path.exists(cached_features_file+"cache-train-{}-{}".format(args.max_len,args.batch_size)) and \
        ...
        lb_path = prefix + 'slot_lbs.txt'
        training_dir = 'training_data'
        dev_dir = 'dev_data'
        if args.load_part:
            input_train_dir = os.path.join(prefix,training_dir,'seq.in')
            label_train_dir = os.path.join(prefix,training_dir,'slot_lbs.txt')
            input_val_dir = os.path.join(prefix,dev_dir,'seq.in')
            label_val_dir = os.path.join(prefix,dev_dir,'slot_lbs.txt')
